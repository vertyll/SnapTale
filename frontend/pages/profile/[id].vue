    ...
    <div v-if="$profileStore.name"
      ...
      <div class="flex item-center pt-2">
        ...
        <button v-else @click="toggleFollow" :class="isFollowing ? 'bg-gray-300' : 'bg-[#bc2cf0] text-white'"
          class="flex items-center rounded-md py-1.5 px-8 mt-3 text-[15px] font-semibold">
      ...
      <div class="flex items-center pt-4">
        ...
        <div class="mr-4">
          <span class="font-bold">{{ $profileStore.followCount }}</span>
...
onMounted(async () => {
  try {
    ...
    isFollowing.value = $profileStore.isFollowing;
  } catch (error) {
    console.log(error);
  }
});
